        ...
        <q-form class="q-gutter-md">
          ...
          <div>
            <q-input square filled clearable v-model="username" type="text" label="Username"
              @focus="usernameTooltip = true" :rules="[required]" />
            <q-tooltip v-model="usernameTooltip" anchor="center right" self="center left">Must be unique and contain at
              least 3 characters</q-tooltip>
          </div>
          <div>
            <q-input square filled clearable v-model="nickname" type="text" label="Nickname"
              @focus="nicknameTooltip = true" :rules="[]" />
            <q-tooltip v-model="nicknameTooltip" anchor="center right" self="center left">Non mandatory</q-tooltip>
          </div>
          <div>
            <q-input square filled clearable v-model="password" type="password" label="Password"
              @focus="passwordTooltip = true" :rules="[required]" />
            <q-tooltip v-model="passwordTooltip" anchor="center right" self="center left">Must contain at least ?
              characters</q-tooltip>
          </div>
        </q-form>
      </q-card-section>
